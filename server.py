...
class NetworkClientController(pb.Root):
        ...
        #----------------------------------------------------------------------
        def remote_UnitShootBulletRequest(self, direction):
...
class TextLogView:
        ...
        def Notify(self, event):
                ...
                if isinstance( event, UnitSpawnEvent ):
                        print(event.name, " at ", event.unit.current_x,' , ', event.unit.current_y)
...
def main():
    ...
    Cntrlr_clientController = NetworkClientController( evManager )
    game = Game( evManager )
